...
dat1 <- read.csv("paternity_birds_mono.csv")
...
birds_1a <- read.csv("paternity_birds_mono.csv", header = TRUE)
...
pdf(file = paste0("forest_bayes_singlerun-w-labels.pdf"), width = 10, height = 20)
print(
  ...
  quote = FALSE,
  text(-2.5, 106, "Species",  pos=4),
  text(2, 106, expression(paste(p[B], " - p [95% CI]")), pos=2)
)
dev.off()
...
res1$vi.f <- res1$vi.f[1:53]
res1$yi.f <- res1$yi.f[1:53]
res1$slab <- bird_species[1:53]

# part1.species <- birds_singlerun[order(birds_singlerun$avgbrood),]$species[1:43]

## Species should already be ordered by avgbrood and labelled correctly - see above
pdf(file = paste0("forest_bayes_singlerun1-w-labels.pdf"), width = 10, height = 12)
print(
  forest(res1, #slab = part1.species, 
         ...
         xlab = expression(paste(p[B], " - p"))),
  quote = FALSE,
  text(-2.5, 55, "Species",  pos=4),
  text(2, 55, expression(paste(p[B], " - p [95% CI]")), pos=2)
)
dev.off()

## Part 2
res2 <- paternity.meta.bayes
res2$vi.f <- res2$vi.f[54:104]
res2$yi.f <- res2$yi.f[54:104]
res2$slab <- bird_species[54:104]

# part2.species <- birds_singlerun[order(birds_singlerun$avgbrood),]$species[44:86]

pdf(file = paste0("forest_bayes_singlerun2-w-labels.pdf"), width = 10, height = 12)
print(
  forest(res2, #slab = part2.species, 
         ...
         xlab = expression(paste(p[B], " - p"))),
  quote = FALSE,
  text(-2.5, 53, "Species",  pos=4),
  text(2, 53, expression(paste(p[B], " - p [95% CI]")), pos=2)
)
dev.off()
...
paternity.meta.k
pdf(file = paste0("forest_bayes_singlerun_k-mod.pdf"), width = 10, height = 20)
print(
