...
dat1 <- read.csv("paternity_birds_fp.csv")
...
birds_1a <- read.csv("paternity_birds_fp.csv", header = TRUE)
...
pdf(file = paste0("forest_bayes_singlerun-w-labels.pdf"), width = 12, height = 20)
print(
  ...
  quote = FALSE,
  text(-2.5, 111, "Species",  pos=4),
  text(2, 111, expression(paste(p[B], " - p [95% CI]")), pos=2)
)
dev.off()
...
res1$vi.f <- res1$vi.f[1:54]
res1$yi.f <- res1$yi.f[1:54]
res1$slab <- bird_species[1:54]

# part1.species <- birds_singlerun[order(birds_singlerun$avgbrood),]$species[1:54]

## Species should already be ordered by avgbrood and labelled correctly - see above
pdf(file = paste0("forest_bayes_singlerun1-w-labels.pdf"), width = 11.5, height = 12)
print(
  forest(res1, #slab = part1.species, 
         ...
         xlab = expression(paste(p[B], " - p"))),
  quote = FALSE,
  text(-2.5, 56, "Species",  pos=4),
  text(2, 56, expression(paste(p[B], " - p [95% CI]")), pos=2)
)
dev.off()

## Part 2
res2 <- paternity.meta.bayes
res2$vi.f <- res2$vi.f[55:109]
res2$yi.f <- res2$yi.f[55:109]
res2$slab <- bird_species[55:109]

# part2.species <- birds_singlerun[order(birds_singlerun$avgbrood),]$species[55:108]

pdf(file = paste0("forest_bayes_singlerun2-w-labels.pdf"), width = 11.5, height = 12)
print(
  forest(res2, #slab = part2.species, 
         ...
         xlab = expression(paste(p[B], " - p"))),
  quote = FALSE,
  text(-2.5, 57, "Species",  pos=4),
  text(2, 57, expression(paste(p[B], " - p [95% CI]")), pos=2)
)
dev.off()

# ## Part 3
# res3 <- paternity.meta.bayes
# res3$vi.f <- res3$vi.f[73:108]
# res3$yi.f <- res3$yi.f[73:108]
# res3$slab <- bird_species[73:108]
# 
# # part3.species <- birds_singlerun[order(birds_singlerun$avgbrood),]$species[163:243]
# 
# pdf(file = paste0("forest_bayes_singlerun3-w-labels.pdf"), width = 11.5, height = 9)
# print(
...
#          xlab = expression(paste(p[B], " - p"))),
#   quote = FALSE,
#   text(-2.5, 84, "Species",  pos=4),
#   text(2, 84, expression(paste(p[B], " - p [95% CI]")), pos=2)
# )
# dev.off()
...
paternity.meta.k
pdf(file = paste0("forest_bayes_singlerun_k-mod.pdf"), width = 12, height = 20)
print(
...
g4 <- ggplot() +
  ...
  labs(x="Brood size", y="Number of mates/sires") +
  scale_x_continuous(breaks = c(seq(1,10, by = 1)) , limits = c(1,10)) +
  scale_y_continuous(breaks = c(seq(0,11, by = 2)) ,limits = c(0,11)) +
