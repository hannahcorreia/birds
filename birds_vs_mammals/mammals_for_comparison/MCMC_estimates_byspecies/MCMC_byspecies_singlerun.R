...
save(MCMC_summary, file = paste0("phyloMCMC_summary_singlerun.rda"))
...
ggsave("p_phylo_singlerun.eps", plot = g1a, device = cairo_ps, width = 11, height = 7.5, dpi = 320)
...
save(MCMC_resids, file = "phyloMCMC_resids.rda")
write.csv(MCMC_resids, file = "phyloMCMC_resids.csv")
...
g1b <- ggplot() +
  ...
  scale_x_continuous(breaks = c(seq(2,10, by = 1)) , limits = c(2,10)) +
  theme_bw(base_size = 16)
ggsave("p_CL_phylo.eps", plot = g1b, device = cairo_ps, width = 11, height = 7.5, dpi = 320)
...
pdf(file = paste0("forest_phylo_singlerun-w-labels.pdf"), width = 11.5, height = 13)
print(
  ...
  quote = FALSE,
  text(-2.5, 66, "Species",  pos=4),
  text(2, 66, expression(paste(p[B], " - p [95% CI]")), pos=2)
)
dev.off()
...
paternity.meta.k
pdf(file = paste0("forest_phylo_singlerun_k-mod.pdf"), width = 15, height = 12)
print(
...
paternity.meta.k <- rma(resid_pmult, vi, mods = ~ avgbrood, data = mammals_singlerun[!mammals_singlerun$pmult==0 & !mammals_singlerun$pmult==1,])
paternity.meta.k
forest(paternity.meta.k, slab = mammals_singlerun[!mammals_singlerun$pmult==0 & !mammals_singlerun$pmult==1,]$species, 
       order = order(mammals_singlerun[!mammals_singlerun$pmult==0 & !mammals_singlerun$pmult==1,]$avgbrood), cex = 1)
...
g2 <- ggplot() +
  ...
  theme_bw(base_size = 16)
ggsave("R_CL_phylo.eps", plot = g2, device = cairo_ps, width = 11, height = 7.5, dpi = 320)
...
ggsave("phylo_MS-k.eps", plot = g3, device = cairo_ps, width = 12, height = 7.5, dpi = 320)
