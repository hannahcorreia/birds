...
dat1 <- read.csv("paternity_birds_xmono.csv")
...
birds_1a <- read.csv("paternity_birds_xmono.csv", header = TRUE)
...
pdf(file = paste0("forest_bayes_singlerun-w-labels.pdf"), width = 10, height = 8)
print(
  ...
  quote = FALSE,
  text(-2.5, 33, "Species",  pos=4),
  text(2, 33, expression(paste(p[B], " - p [95% CI]")), pos=2)
)
dev.off()
...
paternity.meta.k
pdf(file = paste0("forest_bayes_singlerun_k-mod.pdf"), width = 10, height = 8)
print(
