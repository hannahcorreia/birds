...
pdf(file = paste0("forest_bayes_singlerun-w-labels.pdf"), width = 11.5, height = 35)
print(
...
res1$vi.f <- res1$vi.f[1:81]
res1$yi.f <- res1$yi.f[1:81]
res1$slab <- bird_species[1:81]

# part1.species <- birds_singlerun[order(birds_singlerun$avgbrood),]$species[1:81]

## Species should already be ordered by avgbrood and labelled correctly - see above
pdf(file = paste0("forest_bayes_singlerun1-w-labels.pdf"), width = 11.5, height = 18)
print(
  forest(res1, #slab = part1.species, 
         ...
         xlab = expression(paste(p[B], " - p"))),
  quote = FALSE,
  text(-2.5, 83, "Species",  pos=4),
  text(2, 83, expression(paste(p[B], " - p [95% CI]")), pos=2)
)
dev.off()

## Part 2
res2 <- paternity.meta.bayes
res2$vi.f <- res2$vi.f[82:162]
res2$yi.f <- res2$yi.f[82:162]
res2$slab <- bird_species[82:162]

# part2.species <- birds_singlerun[order(birds_singlerun$avgbrood),]$species[82:162]

pdf(file = paste0("forest_bayes_singlerun2-w-labels.pdf"), width = 11.5, height = 18)
print(
...
res3$vi.f <- res3$vi.f[163:244]
res3$yi.f <- res3$yi.f[163:244]
res3$slab <- bird_species[163:244]

# part3.species <- birds_singlerun[order(birds_singlerun$avgbrood),]$species[163:243]

pdf(file = paste0("forest_bayes_singlerun3-w-labels.pdf"), width = 11.5, height = 18)
print(
...
paternity.meta.k
pdf(file = paste0("forest_bayes_singlerun_k-mod.pdf"), width = 14, height = 35)
print(
